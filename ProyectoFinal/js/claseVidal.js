...
$(function () { //$(document).ready()
    ...
    $('#add').click(function () {
        if (validarDatos() == true) {
            ...
            draw();
            let lastRow = getHighestDataRow(mymodel.rows);
            say(lastRow);
            flashRow(lastRow);
        } else {
            Swal.fire({
                position: 'top-end',
                icon: 'info',
                title: 'Faltan completar datos, por favor verifique.',
                showConfirmButton: false,
                timer: 1300
            })
...
//Get highest intserted row
function getHighestDataRow(myArray) {
    let max = myArray.reduce(function (prev, current) {
        if (+current.rowId > +prev.rowId) {
            return current;
        } else {
            return prev;
        }
    });
    //console.log("DataRow" + max[max.length]);
    return "DataRow" + max.rowId;
}
...
//Toggle combobox open/close status
function openCombo(comboBox) {

    $('.dropdown-toggle').dropdown();
}

//Load countroes from JSON
